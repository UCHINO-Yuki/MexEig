...
void dsygvd(mwSignedIndex *Itype, char *jobs, char *uplo, mwSignedIndex *n, double *A, mwSignedIndex *lda, double *B, mwSignedIndex *ldb, double *W, double *work, mwSignedIndex *lwork, mwSignedIndex *iwork, mwSignedIndex *liwork, mwSignedIndex *info);

void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    if (nrhs < 2)
        mexErrMsgIdAndTxt("dsygvd:inputNotEnough", "Input argument is not enough.");
            ...
            if (mxIsChar(prhs[2])){
                ...
                if (mxGetString(prhs[2], type, (mwSize) typelen) == 0){
                    if (strcmp(type, "matrix") == 0){
                        ...
                        free(C);
                        free(D);
                        mexErrMsgIdAndTxt("dsygvd:inputNotChar", "Input argument is not appropriate.");
                    }
                }
            }
            else{
                free(C);
                free(D);
                mexErrMsgIdAndTxt("dsygvd:inputNotChar", "Input argument must be of type char.");
            }
        }
        free(C);
    }
...
void run_vec(char *jobs, double *A, double *B, double *C, double *D, double *eigenvalue, mwSignedIndex *n) {
    ...
    // Get the length
    dsygvd(&Itype, jobs, &UPLO, n, C, n, D, n, eigenvalue, dammy, &lwork, idammy, &liwork, &info);
    ...
    // Run
    dsygvd(&Itype, jobs, &UPLO, n, C, n, D, n, eigenvalue, work, &lwork, iwork, &liwork, &info);
...
void run_mat(char *jobs, double *A, double *B, double *C, double *D, double *eigenvalue, mwSignedIndex *n) {
    ...
    // Get the length
    dsygvd(&Itype, jobs, &UPLO, n, C, n, D, n, eigtemp, dammy, &lwork, idammy, &liwork, &info);
    ...
    // Run
    dsygvd(&Itype, jobs, &UPLO, n, C, n, D, n, eigtemp, work, &lwork, iwork, &liwork, &info);
