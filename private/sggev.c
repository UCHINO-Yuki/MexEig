...
void sggev(char *JOBVL, char *JOBVR,
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    if (nrhs < 2)
        mexErrMsgIdAndTxt("sggev:inputNotEnough", "Input argument is not enough.");
    ...
    if (nlhs == 1) {
		...
		free(ai);
		free(b);
        mexErrMsgIdAndTxt("sggev:TooManyOutput", "Too many output.");
    }
...
void run_vec(char *jobvl, char *jobvr, float *A, float *B, float *C, float *D, float *VL, float *VR, float *ar, float *ai, float *b, mwSignedIndex *n) {
    ...
    // Get the length
    sggev(jobvl, jobvr, n, C, n, D, n, ar, ai, b, VL, n, VR, n, dammy, &lwork, &info);
    ...
    // Run
    sggev(jobvl, jobvr, n, C, n, D, n, ar, ai, b, VL, n, VR, n, work, &lwork, &info);
